...
def draw_hasse(label_highlight_reaction_dict,dot,list_of_solutions,analyze,label_highlight_overproduction_dict={}, DOs=False, shortform=False, show_species=False, only_show_new=False, show_compartments="length", show_overproduction=True, pdf=True, display_bool=True):
...
def create_Hasse(list_of_solutions,analyze, DOs=False, shortform=False, show_species=False, only_show_new=False,show_compartments="length", show_overproduction=False, analyze_only=False, pdf=True):
    ...
    label_highlight_overproduction_dict={}
    ...
    for i in range(len(list_of_solutions)):
        ...
        if show_overproduction==True:
            label_highlight_overproduction_dict[i]=analyze.overproduction[i][:]
    ...
    for i in range(1,len(list_of_solutions)):
        ...
        for candidate in candidatelist:
            ...
            for faceobject in facedict[candidate_froz]:
                ...
                if show_overproduction:
                    label_highlight_overproduction_dict[j]= [e for e in label_highlight_overproduction_dict[j] if e not in analyze.overproduction[i]]
    ...
    if analyze_only:
        ...
        x=draw_hasse(label_highlight_reaction_dict,dot,list_of_solutions,analyze,label_highlight_overproduction_dict, DOs, shortform, show_species, only_show_new, show_compartments, show_overproduction, pdf)
        return(x)
def create_Hasse_naive(list_of_solutions,RN, DOs=False, shortform=False, show_species=False, only_show_new=False,show_compartments="length", show_overproduction=False,analyze_only=False, pdf=True):
    dot = Digraph(strict=True,comment='Hasse_Diagram')
    #dot.attr( size='8,5')
    cover={}
...
#loop over all knots
   ...
   # draw_hasse(label_highlight_reaction_dict,label_highlight_overprod_dict,dot,list_of_solutions,LOR,DOs, shortform, show_species, show_new, show_compartments, overproduction)
        x=draw_hasse(label_highlight_reaction_dict,label_highlight_overprod_dict,dot,list_of_solutions,RN,DOs, shortform, show_species, only_show_new, show_compartments, show_overproduction, pdf)
        return(x)
